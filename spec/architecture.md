...
状态：基础技术选型已确认（2026-09-19）。业务设计、依赖具体版本与应用实现尚未完成。
...
| 部分 | 已确认选择 / 实施边界 |
| --- | --- |
| 链与网络 | Fuji C-Chain，Chain ID `43113`；RPC 等参数见 [生态文档](ecosystem.md) |
| 应用形式 | Web 单页 Demo，采用单应用结构 |
...
| 自定义智能合约 | 是否需要仍待业务确定；需要时使用 Solidity + Hardhat 开发、测试和部署 |
...
基础工程按以下 Web + C-Chain 边界组织；具体业务模块在功能 Spec 中补充。
...
业务规则使用 Vitest 验证，钱包和 RPC 异常分支可使用模拟测试；关键页面流程使用 Playwright。若开发自定义合约，使用 Hardhat 验证权限、状态变更和回滚条件。真实 Core/Fuji 验收记录网络、交易回执或实际读取结果，不能用模拟 Provider 的通过结果代替。具体测试场景写在功能 Spec，执行结果写在 Roadmap。

## 尚待业务明确

- 目标用户、首个核心流程及其验收标准。
- 哪些操作和数据必须上链，是否需要自定义合约。
